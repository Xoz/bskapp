...
def kallelsesvar(aktivitets_id: str) -> dict:
    ...
    with database('view_players') as (conn,group):
        ...
        rows=rows[:200]
        own_ids={r['id'] for r in conn.execute('SELECT id FROM bsk_hermes.players').fetchall()}
        def member(r):
            return dict(player_id=r['player_id'],name=r['name'],belongs_to_connected_team=r['player_id'] in own_ids)
        groups={key:[member(r) for r in rows if r['response']==key]
                for key in ('accepted','declined','pending')}
        groups['no_response_data']=[member(r) for r in rows if r['response'] not in ('accepted','declined','pending')]
        groups['selected']=[member(r) for r in rows if r['selected']]
        ...
        return response(group,event=event,players=groups,named_counts=named,source_counts=totals,
                        ...
                        truncated=truncated,
                        caveat='Gäster med belongs_to_connected_team=false tillhör inte det anslutna laget. Vid frågor om lån måste laneunderlag användas. Ja betyder tackat ja, inte säker eller registrerad närvaro. Uttagen är ett separat tränarbeslut och kan överlappa nej/obesvarat. Saknade svar betyder inte nej eller ej kallad. Om namnantalen inte stämmer med källtotalerna, eller totaler saknas, redovisa att namnlistan kan vara ofullständig. data_updated_at är senaste sparade radändring, inte garanterad tid för en fullständig synk.')


@mcp.tool(annotations=READ)
def laneunderlag(mal_match_id: int | None = None) -> dict:
    """Obligatoriskt underlag för vilka Gulspelare som kan lånas ut till Grön. Utan id: hitta Gröns målmatcher kommande sju dagar. Ange sedan exakt id för egna spelare, åtaganden i alla lag, batteri före/efter/lägst, tidsmarginal och synktäckning. Använd aldrig enbart selected eller kallelsesvar för att anta ledighet. Gäster är inte Gulspelare. Detta är planering, inte prestationsranking. Inga kallelser skickas."""
    if mal_match_id is not None:
        positive(mal_match_id)
    # Befintlig kontobehörighet plus serverns separata målmatch-/spelarkontroll.
    with database('view_matches'):
        pass
    try:
        config=json.loads(Path('/etc/bsk-hermes/loans.json').read_text())
        url='http://127.0.0.1:3001/api/hermes/loans'
        if mal_match_id is not None:
            url+='?matchId='+str(mal_match_id)
        request=urllib.request.Request(url,headers={'Authorization':'Bearer '+config['token']})
