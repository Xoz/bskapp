...
## Låneunderlag via MCP – 2026-09-11

Nionde läsverktyget `laneunderlag` listar utan argument behöriga målmatcher
kommande sju dagar. Med `mal_match_id` ger det endast aktiva Gulspelare med
medlemskap giltigt på målmatchens datum, åtaganden över laggränser, appens
gemensamma procentbatteri och kategoriserade kalenderkonflikter. Gäster i
`kallelsesvar` markeras `belongs_to_connected_team=false` och får inte beskrivas
som Gulspelare. Medlemsmarkeringen i kallelsesvar gäller nuläget; låneverktyget
kontrollerar målmatchens datum.

Appens GET `/api/hermes/loans` är separat autentiserad och kan inte skriva.
MCP använder fast loopbackadress port 3001. `/etc/bsk-hermes/loans.json` innehåller
slumpad separat token, userId, sourceGroupId och targetGroupIds; mode 0640,
root:bsk-hermes. Inga tokenvärden i Git. Konto måste fortfarande vara aktivt,
vara staff och ha view_players/view_matches/view_statistics. Källag och konto
måste stämma med bsk_hermes.binding; både källag och mållag verifieras mot
aktuella gruppbehörigheter vid varje anrop. Varken databastabellgrants eller
åtkomst till andra spelares utvecklingsprofiler utökas.

`lib/matchSpaceReader.ts` delar den befintliga dataläsningen med webbappen;
`forecastMatchSpace` och `batteryPercent` används direkt. Ingen duplicerad
Python-simulering. Cuper utesluts ur batteri/ordinarie antal men redovisas som
separata kalenderåtaganden. Avsaknad av kallelse bevisar aldrig ledighet.
Tidsmarginalen är uttryckligen ett antagande: 30 min samling + 30 min resa +
10 min pauser, totalt 70 min. Det är inte en verifierad färdväg; en konflikt
benämns därför upptagen enligt tidsantagande. Återkommande egen match senare
samma dag ingår i batteriprognosen men gör inte automatiskt personen upptagen
hela dagen. Bristande synktäckning kräver kontroll före lån.

Aktivering: publicera huvudappen först; skapa skyddad tokenfil på VPS med Gul
som källa och Grön som enda tillåtet mållag; provkör endpointen med rätt/fel
token. Paketera ny MCP-release, kör test_integration.py och test_loans.py,
växla kodlänk atomiskt. Lägg enbart laneunderlag till privata profilens BSK
allowlist. Uppdatera BSK-instruktionen att använda verktyget för lånefrågor och
ladda om bara privata gatewayen. Verifiera stdio och en vanlig modellfråga utan
utskick. Gamla match-/träningsrapporter är oförändrade.
