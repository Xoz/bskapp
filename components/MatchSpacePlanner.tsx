...
export default function MatchSpacePlanner({ players }: {players: {id:number; name:string; team:string; input:SpaceInput; status:string | null}[]}) {
  ...
  return <div className="space-y-4">
    ...
    <details className="core-panel p-4 text-sm">
      ...
      <p className="mt-2">Uppskattning: saknad speltid räknas som hela matchen och träning som 60 minuter. Historiken börjar för 28 dagar sedan med antaget fullt batteri. Andra idrotter och saknad närvaro ingår inte. Ändrad speltid här är bara ett scenario och sparas inte i matchplanen.</p>
    </details>
    {rows.map(({id,name,team:playerTeam,input,status,forecast:f}) => <article key={id} className="core-panel p-4 space-y-3">
      ...
      <div className="flex justify-between items-center gap-3 flex-wrap"><label className="text-sm">Prova speltid (min)
        <input aria-label={`Prova speltid för ${name}`} className="input ml-2" style={{width:"5rem"}} type="number" min={0} max={input.target!.duration} value={minutes[id] ?? input.target!.minutes} onChange={e => setMinutes(previous => ({...previous,[id]:Math.max(0,Math.min(input.target!.duration, Number(e.target.value)))}))}/>
