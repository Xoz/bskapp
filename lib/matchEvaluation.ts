...
export async function getMatchEvaluationWorkspace(matchId: number, contributorType: "coach" | "invite", contributorId: string): Promise<MatchEvaluationWorkspace | null> {
  ...
  const players = await all<MatchEvaluationPlayer>(
    `SELECT p.id, p.name, p.jersey_number,
     ...
     ORDER BY lower(p.name)`, [matchId, contributorType, contributorId, ...rosterIds]
  );
...
export async function getMatchEvaluationStatus(matchId: number) {
  ...
  const result = await get<{ evaluated: number; contributors: number }>(
    `SELECT COUNT(DISTINCT e.player_id)::int AS evaluated,
            COUNT(DISTINCT (e.contributor_type || ':' || e.contributor_id))::int AS contributors
     FROM match_player_evaluations e
     WHERE e.match_id = ? AND e.player_id IN (${marks})`,
    [matchId, ...rosterIds]
  );
  return {
    total: roster.players.length,
    evaluated: result?.evaluated ?? 0,
    contributors: result?.contributors ?? 0,
  };
}
