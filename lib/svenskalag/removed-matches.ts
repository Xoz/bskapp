import type {SyncTeam} from "./model";
...
export async function applyRemovedMatches(sql:ReturnType<typeof postgres>,removed:RemovedMatch[],today:string,dryRun=false,team:SyncTeam="Gul") {
  ...
  await sql.begin(async tx=>{
    for(const item of removed) {
      const rows=await tx`SELECT m.id FROM matches m JOIN groups g ON g.id=m.group_id
        WHERE m.id=${item.matchId} AND g.name=${team} AND g.group_type='subgroup' AND g.active=1
        AND m.date LIKE ${today.slice(0,4)+'%'} AND m.source IN ('calendar','svenskalag_sanktan')
        AND (m.external_uid=${'sanktan:'+item.sourceId} OR (${team}='Gul' AND m.external_uid=${'cal'+item.sourceId+'-40600@svenskalag.se'})
