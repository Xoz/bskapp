...
describe.skipIf(!process.env.BSK_SYNC_TEST_DATABASE_URL)('spelarlistans matchstatistik', () => {
  it('avgränsar år, lag och cupgrupper samt skiljer kallelse från uttagning', async () => {
    ...
    try {
      await sql.unsafe(`CREATE TEMP TABLE groups(id int, name text, group_type text, parent_id int);
        ...
        INSERT INTO groups VALUES(1,'Gul','subgroup',NULL),(2,'Cup','match',1),(3,'Grön','subgroup',NULL),(4,'Cupfinal','match',2);
      ...
      expect(await read('Gul')).toEqual([{ player_id: 1, match_count: 4, callup_count: 4 }]);
      expect(await read('Grön')).toEqual([{ player_id: 1, match_count: 1, callup_count: 1 }]);
      expect(await read(null)).toEqual([{ player_id: 1, match_count: 6, callup_count: 6 }]);
