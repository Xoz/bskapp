...
export type SpaceReader = {all: typeof Database.all; get: typeof Database.get; canAccessGroup: (id: number | null) => Promise<boolean>};
/** Internt läslager. Anroparen måste verifiera spelaråtkomst före anrop. */
export async function loadMatchSpaceInputs(db: SpaceReader, playerIds: number[], targetMatchId?: number, now = Date.now()): Promise<Map<number, SpaceInput>> {
  const {all,get} = db;
  ...
  if (targetMatchId != null && (!target || !(await db.canAccessGroup(target.group_id)))) throw new Error("Matchen är inte tillgänglig.");
