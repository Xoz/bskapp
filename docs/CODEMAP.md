...
| **Gemensamt matchnav i native och webb** (Översikt, Trupp, Matchcenter och Utvärdera; behörighets-, lag- och matchstatusstyrt) | `ios/BSK/Views/{ActivityViews,MainSplitView}.swift`, `ios/BSK/AppModel.swift`, `app/(skyddad)/matcher/[id]/`, `lib/services/{development,matchEvaluationMobile,mobileLive}.ts`, `lib/matchRoster.ts` |
| **Individuellt matchutrymme / kallelse och lån** | `lib/matchSpace.ts` (tidsbaserad simulering), `lib/matchSpaceData.ts` (kanoniska matcher, deltagande, träning), `lib/matchSpaceActions.ts` (behörighet + kapacitet i `settings`, nyckel `match_space_capacity:<id>`), `components/MatchSpace{Planner,Profile}.tsx`, `app/(skyddad)/matcher/[id]/matchutrymme/`, Idag och SelectionEditor. Se `docs/MATCH_SPACE.md`. |
| **Transparent uttagningsstöd** (läser matcher från `matches`; kallelse och svar från `match_roster`; uttagning är endast ibockad `selected` eller tomt; Gul Sanktan använder samma arbetsyta från webbens matchnav och native) | `lib/selectionSupport.ts`, `lib/developmentCore.ts` (getSelectionMatches/getSelectionWorkspace), `lib/coreActions.ts` (saveDevelopmentSelection), `app/(skyddad)/uttagning/`, `app/(skyddad)/matcher/[id]/{page,laguttagning/page}.tsx` |
...
- **callupWorkbook.ts / services/callupWorkbookImport.ts**: tolkar Excel-exporten `Kallelser, svar & närvaro`; matchdelen uppdaterar Gul och Grön, medan endast Guls träningar från idag till 14 dagar framåt normaliseras med individuella kallelser och svar.
...
`match_events` (inkl. lokal `reporter_key` för egen ångra + `idempotency_key` för offline-replay-skydd), `match_reporters`, `live_rate_limits`, `match_roster` (kanonisk kallelse/svar/uttagning/startplacering; uttagning lagras endast som `selected` eller `NULL`), `match_subs` (inkl. `idempotency_key`), `match_evaluation_invites`, `match_player_evaluations`,
...
Matchutvärdering 1–5: `components/MatchEvaluationForm.tsx` + `match-ratings.css` visar alla deltagare samtidigt. `lib/matchRating.ts` innehåller `RATING_RULES`, `assessMatchLevel` och validerade SQL-uttryck för numeriska svar; tester i `lib/matchRating.test.ts`. `lib/matchEvaluation.ts` läser numeriska/äldre svar och grupperar per match. `MatchEvaluationTrend.tsx` visar nivåunderlaget i profilens befintliga komponentplats. Migration 0024 i `lib/db.ts`, befintliga coach-/inbjudningsactions och `services/matchEvaluationMobile.ts` delar lagringen. Fem tränarbedömda nivåer via `playerLevelPreferences.ts`, `coreActions.ts` och `PlayerSelectionPreferencesForm.tsx`. Browserprov: `scripts/test-match-ratings.mjs`. Se `docs/MATCH_RATINGS.md` för regler, data och samordning med profilarbetet.
