...
| **BSK → Planlinjen sessionsbrygga** | `app/api/auth/coach-bridge/route.ts`, `lib/coachBridge.ts`, `coach-platform/src/lib/{bridge-auth,coach-session}.ts`, `coach-platform/src/proxy.ts`, `coach-platform/deploy/nginx-coach.conf.example` |
