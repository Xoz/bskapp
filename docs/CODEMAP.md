...
| **Transparent uttagningsstöd** (läser matcher från `matches`; kallelse och svar från `match_roster`; uttagning är endast ibockad `selected` eller tomt) | `lib/selectionSupport.ts`, `lib/developmentCore.ts` (getSelectionMatches/getSelectionWorkspace), `lib/coreActions.ts` (saveDevelopmentSelection), `app/(skyddad)/uttagning/` |
