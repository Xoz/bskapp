...
2026-09-11: Batteriraden samlar namn, stapel och procent på en rad även på mobil. Grönt upp till lägsta prognos, dämpade grågröna diagonala streck mellan prognos och nuläge; guldfärgen borttagen. Långa namn förkortas visuellt med ellips. Samma komponent används i översikt, profil och matchprognos; beräkningar oförändrade.

2026-09-11: Hermes svar om lån granskades mot produktion och aktiv MCP-kod. Integrationens Gulscope saknar Gröns målmatch, spelaråtaganden över laggränser och batteriprognos; kallelsesvar skiljer inte gäster från egna lagmedlemmar. Föreslagen rättning: särskilt skrivskyddat låneunderlag med gemensam appberäkning, medlemskap, separata svar/uttagning, tidsmarginaler och synktäckning. Ingen åtkomst eller drift ändrad. Detaljplan och acceptansprov: BSK-repots docs/HERMES_LOAN_AUDIT.md.

2026-09-11: Låneunderlag implementerat via Hermes MCP `laneunderlag` och autentiserad läsroute `/api/hermes/loans`. Samma `matchSpaceReader`/simulering som appen, målmatch en gång, kandidater från giltigt aktivt medlemskap i Gul, åtaganden över laggränser och gästmarkering i kallelsesvar. Separat VPS-token med konto-/lagbindning och kontroller per anrop. Tidsmarginal 70 minuter är synligt antagande; ingen uppmätt restid. Cuper undantas ur batteri/antal men visas som kalenderåtaganden. 209 lokala tester inklusive PostgreSQL samt 16 Python/DB-MCP-tester godkända före aktivering. Driftverifiering dokumenteras separat efter publicering.
