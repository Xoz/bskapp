...
import { IconArrowRight, IconPlayers, IconPitch } from "@/components/Icons";
...
export default async function TodayPage() {
  ...
  const nextMatch = yellowUpcoming.find((activity) => activity.is_upcoming);
  ...
  const visiblePlayers = previewPlayers;
      ...
      <div className="bsk-today-grid">
        <div className="bsk-today-main">
          ...
          {canPlayers && <section><div className="core-section-head"><h2 className="core-section-title">Spelarutveckling</h2><Link href="/spelare" className="btn-secondary btn-sm">Visa alla</Link></div>
            <div className="core-panel bsk-link-list">{visiblePlayers.length ? visiblePlayers.map(({ player }) => <Link key={player.id} href={`/spelare/${player.id}`} className="bsk-link-row"><Avatar name={player.name} jersey={player.jersey_number} size={40} /><span className="min-w-0 flex-1"><strong>{player.name}</strong><small>Utvecklingsträd och samtal</small></span><IconArrowRight width={18} height={18} /></Link>) : <p className="p-5" style={{ color: "var(--ink-secondary)" }}>Inga spelare att visa ännu.</p>}</div>
        ...
        <aside className="bsk-today-aside">
          <section id="veckans-matcher"><div className="core-section-head"><h2 className="core-section-title">Den här veckan</h2></div><div className="core-list">{yellowUpcoming.map((activity) => <CoreActivityCard key={activity.id} activity={activity} href={matchHref(activity)} />)}</div>
